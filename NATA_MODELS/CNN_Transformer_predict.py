...
Loads a trained CNN-Transformer model, runs inference on the test set,
...
class VideoSequenceDataset(Dataset):
    def __init__(self, video_folder, label_folder, video_ids, sequence_length=30,
                 stride=5, img_size=(76, 142), behavior_names=None):
        ...
        for video_id in tqdm(video_ids, desc="Indexing videos"):
            ...
            label_path = os.path.join(label_folder, f"{video_id}.csv")
            ...
            try:
                ...
                for col in behavior_names:
                    if col not in labels_df.columns:
                        labels_df[col] = 0
                video_labels = labels_df[behavior_names].values.argmax(axis=1)
            ...
            for start_idx in range(0, min(total_frames, len(video_labels)) - sequence_length + 1, stride):
                self.sequence_info.append((video_id, start_idx))
                self.labels.append(int(video_labels[start_idx]))
...
def predict_per_video(model, dataloader, device, behavior_names):
    ...
    + postprocessing. Returns dict: {video_id: np.array of predicted class indices}
    """
    model.eval()
    ...
    # Sort frames and apply postprocessing
    result = {}
...
if __name__ == "__main__":
    ...
    available = [v for v in test_video_ids
                 if os.path.exists(os.path.join(VIDEO_FOLDER, f"{v}.mp4"))]
    missing = set(test_video_ids) - set(available)
    if missing:
        print(f"Warning: videos not found on disk: {missing}")

    # Build dataset & loader
    dataset = VideoSequenceDataset(
        VIDEO_FOLDER, LABEL_FOLDER, available,
