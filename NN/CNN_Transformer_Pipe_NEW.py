...
class CNNTransformerClassifier(nn.Module):
    ...
    3. Simplified classification head: Dropout + single Linear layer
    """
...
if __name__ == "__main__":
    ...
    for cls_idx in range(num_classes):
        count = class_counts.get(cls_idx, 0)
        class_weights[cls_idx] = (total_samples / (num_classes * count)) ** 0.7 if count > 0 else 1.0
    ...
    sns.heatmap(cm_train_pct[:, ::-1], annot=True, fmt='.1f', cmap='Greens',
                xticklabels=behavior_names[::-1], yticklabels=behavior_names)
    ...
    plt.figure(figsize=(10, 8))
    sns.heatmap(cm_test_pct, annot=True, fmt='.1f', cmap='Blues',
    ...
    plot_behavior_indices = list(range(1, num_classes))
